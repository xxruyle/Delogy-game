...
class TileManager {
  public:
    ...
    int renderDistance = LuaGetInt("RENDER_DISTANCE", "scripts/game_settings.lua");

    TileManager(int random_seed) : world_seed(random_seed) { std::cout << "World seed: " << random_seed << std::endl; };

    std::vector<TileChunk> chunks;
    std::vector<int> updatedChunks;

    std::unordered_map<Vector2, std::vector<entt::entity>, Vector2Util, Vector2Util> entityPositionCache;
    ...
    void checkPlayerInteraction(Camera2D& camera, UI& ui, InventoryC& playerInventory,
