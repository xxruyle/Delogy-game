...
class CartManager {
  public:
    int cartCount = 0;
    void createCart(Vector2 position);
    void changeCartDirection(PositionC& position, OrecartC& orecart, PhysicsC& physics, int itemUnder, int prevItemUnder, int futureItemUnder);
    void changeCartVelocity(PhysicsC& physics, OrecartC& orecart);
    void changeCartPosition(PositionC& position, PhysicsC& physics);
    void updateCarts(TileManager& tileManager);
    void getPlayerInteraction(InventoryC& inventory, HotBarC& hotBar, Camera2D& camera);
    void update(TileManager& tileManager, Scene& scene);

  private:
    int getDirectionMultiplier(int direction);
    std::unordered_set<int> getValidRails(int rail,
    ...
    Vector2 getNearSideCartBorder(PositionC& position,
