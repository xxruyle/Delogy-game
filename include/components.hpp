...
struct AnimationC {
    ...
    float animationSpeed = LuaGetFloat("ANIMATION_SPEED", "scripts/game_settings.lua");
    ...
    AnimationC(Rectangle origin, unsigned int numframes, unsigned int numFramesPerRow) : frameOrigin(origin), numFrames(numframes), framesPerRow(numFramesPerRow)
    {
        curFrameSrc = 3;
        for (unsigned int i = 0; i < numFrames; i++) {
            frameSrcs.push_back((Rectangle{(float)(origin.x), (float)(origin.y + i * (origin.height + ATLAS_SPRITE_PADDING)), (float)origin.width, (float)origin.height}));
        }
...
struct InventoryC {
    std::vector<int> slots = {RAIL_V, CART, STORAGE_BOX, MUSHROOM_PURPLE, CART, CART, MUSHROOM_PURPLE, CART, MUSHROOM_PURPLE, CART, RAIL_V};

    std::vector<int> stacks = {1, 2, 1, 1, 1, 1, 1, 2, 1, 1, 2};
...
enum actionType { EATING, RESTING, SOCIALIZING, PLAY, DEFENSE, SEARCHING, NONE };
